...
//----------------------------------------------------------
{
	...
	pDoc = pInfo->pModDoc;
	CSoundFile &sndFile = pDoc->GetrSoundFile();
	// Create headers
	s[0] = 0;
	...
	if (sndFile.GetType() & (MOD_TYPE_IT|MOD_TYPE_XM|MOD_TYPE_MPT))
	{
...
//----------------------------------------------------------
{
	...
	if(info->item.pszText != nullptr && modDoc != nullptr)
	{
		...
		case MODITEM_SAMPLE:
			if(modItemID <= sndFile.GetNumSamples() && sndFile.GetSample((SAMPLEINDEX)modItemID).name != info->item.pszText)
			{
				mpt::String::CopyN(sndFile.GetSample((SAMPLEINDEX)modItemID).name, info->item.pszText, modSpecs.sampleNameLengthMax);
				modDoc->SetModified();
